...
public:
     ...
     */
    LogFormatter(const std::string &pattern = "%d{%Y-%m-%d %H:%M:%S} [%rms]%T%t%T%N%T%F%T[%p]%T[%c]%T%f:%l%T%m%n");//有个默认参数
     ...
     * @return 格式化日志流
     */
    std::ostream &format(std::ostream &os, LogEvent::ptr event); //疑问，为啥这里需要定义一个返回流的函数
...
public:
     ...
     * @param[in] event 日志事件
     */
    LogEventWrap(Logger::ptr logger, LogEvent::ptr event); //将日志器和日志事件包装在一起
