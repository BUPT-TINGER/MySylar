...
#define XX(level, v) if(str == #v) { return LogLevel::level; } //变量前面加一个#的意思是将变量字符串化
    XX(FATAL, fatal);
    XX(ALERT, alert);
    XX(CRIT, crit);
    XX(ERROR, error);
...
public:
    ...
    void format(std::ostream& os, LogEvent::ptr event) override { //override是覆盖的意思
        struct tm tm;
...
void LogFormatter::init() {
    ...
    while(i < m_pattern.size()) {
        ...
        if(c == "%") {//开始处理模板
            if(parsing_string) {//修改状态
                if(!tmp.empty()) {//先把正常的字符存储，有点像处理缓冲区
                    patterns.push_back(std::make_pair(0, tmp));//0表示正常字符
                }
                tmp.clear();
                parsing_string = false; // 在解析常规字符时遇到%，表示开始解析模板字符，在此处修改状态
                // parsing_pattern = true;
                i++;
                continue;
            } else {//如果parsing_string不为真则不用修改标志位开始处理模板
                patterns.push_back(std::make_pair(1, c));//存进去
...
#define XX(str, C)  {#str, [](const std::string& fmt) { return FormatItem::ptr(new C(fmt));} }//#str代表把str符号化，变成string类型
