...
/**
 * @brief ConfigVar的管理类
 * @details 提供便捷的方法创建/访问ConfigVar mark
 */
class Config {
public:
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string &name,
                                             const T &default_value, const std::string &description = "") { //typename表示这是一个类型名
        RWMutexType::WriteLock lock(GetMutex());
        auto it = GetDatas().find(name);
        if (it != GetDatas().end()) {
            auto tmp = std::dynamic_pointer_cast<ConfigVar<T>>(it->second); //基类要调用继承类的相关内容就得用dynamic_pointer_cast
            if (tmp) {
