...
IOManager::IOManager(size_t threads, bool use_caller, const std::string &name) //mark
...
IOManager::~IOManager() {
    ...
    for (size_t i = 0; i < m_fdContexts.size(); ++i) {//为啥这里需要手动删除？？
    //其他的都是局部变量，而该变量申请了指针，如果不delete就只要这个指针对象会被删除，但是其对应申请的空间就不会被删除，会引起内存泄漏
        if (m_fdContexts[i]) {
...
void IOManager::contextResize(size_t size) {
    ...
    for (size_t i = 0; i < m_fdContexts.size(); ++i) {
        if (!m_fdContexts[i]) {
            m_fdContexts[i]     = new FdContext; //因为不知道从多少开始时新增的，所以遍历一次新分配空间，其实可以记录一下上一次的容量节省时间
            m_fdContexts[i]->fd = i;
        }
    }
}
...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
    ...
    if ((int)m_fdContexts.size() > fd) { //检查一下，[fd]直接溢出了
        fd_ctx = m_fdContexts[fd];
        lock.unlock();
    } else {
        lock.unlock();
        RWMutexType::WriteLock lock2(m_mutex);
        contextResize(fd * 1.5); //有点类似于vector扩容
        fd_ctx = m_fdContexts[fd];
    }
    ...
    // 找到这个fd的event事件对应的EventContext，对其中的scheduler, cb, fiber进行赋值  fd和总的event之间的关系多少有点混乱
    //fd里的event进行了简化，只有读写无三种事件，和epoll的event不太一样
...
bool IOManager::cancelAll(int fd) {
    ...
    epoll_event epevent;
    epevent.events   = 0;  //删除全部事件就是，把该fd的事件集合置0然后，这个fd_ctx是啥有点没懂，还得看见epoll
    epevent.data.ptr = fd_ctx;//events的ptr指的是和fd相关的一些用户资源，那这删都删了还指定啥啊？？？

    int rt = epoll_ctl(m_epfd, op, fd, &epevent); //因为版本问题，最后一个参数以前必须要传一个非空指针，但是现在可以直接写成null
    if (rt) {
        SYLAR_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", "
                                  << (EpollCtlOp)op << ", " << fd << ", " << (EPOLL_EVENTS)epevent.events << "):"
                                  << rt << " (" << errno << ") (" << strerror(errno) << ")";
...
void IOManager::idle() {
    ...
    while (true) {
        ...
        for (int i = 0; i < rt; ++i) {
            ...
            int rt2 = epoll_ctl(m_epfd, op, fd_ctx->fd, &event); //第一个参数是create的固定返回值，第二个是指定到底是删除还是添加，第三个是要操作的fd，后面是fd要进行的哪些事件
            if (rt2) {
                SYLAR_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", "
