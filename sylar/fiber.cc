...
/// 全局静态变量，用于生成协程id
static std::atomic<uint64_t> s_fiber_id{0}; //原子操作，可以用来当多线程的计数
/// 全局静态变量，用于统计当前的协程数
...
using StackAllocator = MallocStackAllocator; //取了一个别名
...
Fiber::Fiber() {
    ...
    if (getcontext(&m_ctx)) {  //getcontext成功返回0，失败返回-1，而if条件非0都看作1
...
Fiber::Fiber(std::function<void()> cb, size_t stacksize, bool run_in_scheduler)
    ...
    makecontext(&m_ctx, &Fiber::MainFunc, 0); //创建协程上下文
...
Fiber::~Fiber() {
    ...
    if (m_stack) {
        ...
        SYLAR_ASSERT(m_state == TERM); //不是结束状态就出错中断了
...
void Fiber::yield() {
    ...
    SetThis(t_thread_fiber.get());//必须将t_fiber时刻指向正在运行的协程（即线程里面的主协程），同时方便后续进行相对应的上下文切换
    if (m_state != TERM) {
        m_state = READY;
    }
