...
Scheduler::Scheduler(size_t threads, bool use_caller, const std::string &name) {
    ...
    if (use_caller) {
         ...
         */
        m_rootFiber.reset(new Fiber(std::bind(&Scheduler::run, this), 0, false)); //这里是之前就为caller线程创立了相对应的线程，所以只需要绑定一下就行了

        sylar::Thread::SetName(m_name);
        t_scheduler_fiber = m_rootFiber.get();
        m_rootThread      = sylar::GetThreadId();
        m_threadIds.push_back(m_rootThread);//这些操作都是记录然后保存
    } else {
        m_rootThread = -1;
    }
...
Scheduler::~Scheduler() {
    ...
    if (GetThis() == this) {//如果是调度器本身析构了，就得自己处理调度器将其置空
        t_scheduler = nullptr;
    }
}
...
void Scheduler::idle() { //因为run那是一个while(1)的循环，所以就进idle就让出然后又在循环中进入然后又让出，忙等待
...
void Scheduler::run() {
    ...
    while (true) {
        ...
        bool tickle_me = false; // 是否tickle其他线程进行任务调度
        {
            ...
            tickle_me |= (it != m_tasks.end()); //位或存储到tickle_me中
        }
